...
class MainActivity : AppCompatActivity() {
    ...
    private fun fetchMovies() {
        ...
        // Using the "popular" endpoint instead of "now_playing"
        val url = "https://api.themoviedb.org/3/movie/popular?api_key=a07e22bc18f5cb106bfe4cc1f83ad8ed"

        client.get(url, object : JsonHttpResponseHandler() {
            override fun onSuccess(statusCode: Int, headers: Headers?, json: JSON) {
                val gson = Gson()
                val movieType = object : TypeToken<List<Movie>>() {}.type
                val movies: List<Movie> = gson.fromJson(json.jsonObject.getJSONArray("results").toString(), movieType)
